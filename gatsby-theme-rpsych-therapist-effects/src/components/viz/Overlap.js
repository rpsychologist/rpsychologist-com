...
const margin = { top: 70, right: 20, bottom: 45, left: 20 };
...
const OverlapChart = (props) => {
  ...
  const aspect = width < 450 ? 0.5 : 0.35;
  ...
  // Data
  const data1 = useMemo(() => genData(M0, SD, x), [M0, SD, w, reset, rescale]);

  // Axes min and max
  const xMax = useMemo(() => M1 + SD * 3, [reset, rescale]);
  const xMin = useMemo(() => M0 - SD * 3, [reset, rescale]);
  ...
  const PathDist1 = useMemo(() => linex(data1.data), [SD, M0, w, reset, rescale]);
